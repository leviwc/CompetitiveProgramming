...
void solve() {
    ...
    for (int i = 0; i < n - 1; i++) {
        b -= monster[i].first *
             (monster[i].second / a + (monster[i].second % a != 0));
        if (b <= 0) {
            cout << "NO" << endl;
            return;
        }
    }
    int rod = min(monster[n - 1].second / a + (monster[n - 1].second % a != 0),
                  b / monster[n - 1].first + (b % monster[n - 1].first != 0));
    monster[n - 1].second -= rod * a;
