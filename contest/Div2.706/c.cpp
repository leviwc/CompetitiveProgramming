...
void solve() {
    ...
    for (int i = 0; i < 2 * n; i++) {
        ...
        cin >> a >> b;
        if (b == 0) {
            diamond.push_back(abs(binpows(a, 2)));
        } else {
            miner.push_back(abs(binpows(b, 2)));
        }
    }
    ...
    double ans = 0, ans2 = 0;
    for (int i = 0; i < n; i++) {
        ans += (sqrt(miner[i] + diamond[n - i - 1]));
    }
    for (int i = 0; i < n; i++) {
        ans2 += (sqrt(miner[i] + diamond[i]));
    }
    cout << fixed << setprecision(9) << min(ans, ans2) << endl;
}
